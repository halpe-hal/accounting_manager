"use client";

import { useState, useTransition, useEffect } from "react";
import { useRouter } from "next/navigation";
import { addExpenseCategory, updateExpenseCategory, deleteExpenseCategory, updateExpenseCategorySortOrder } from "@/app/actions/settings";
import type { ExpenseCategory } from "@/lib/types";

interface Props {
  initial: ExpenseCategory[];
}

export default function ExpenseCategoriesTab({ initial }: Props) {
  const router = useRouter();
  const [items, setItems] = useState(initial);
  useEffect(() => { setItems(initial); }, [initial]);
  const [name, setName] = useState("");
  const [isFixed, setIsFixed] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editName, setEditName] = useState("");
  const [editIsFixed, setEditIsFixed] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [permError, setPermError] = useState<string | null>(null);

  async function handleAdd() {
    if (!name.trim()) return;
    startTransition(async () => {
      const result = await addExpenseCategory(name.trim(), isFixed);
      if (result?.error) { setPermError(result.error); return; }
      setName(""); setIsFixed(false);
      router.refresh();
    });
  }

  function startEdit(c: ExpenseCategory) {
    setEditingId(c.id);
    setEditName(c.name);
    setEditIsFixed(c.is_fixed);
  }

  async function handleUpdate(id: number) {
    if (!editName.trim()) return;
    const result = await updateExpenseCategory(id, editName.trim(), editIsFixed);
    if (result?.error) { setPermError(result.error); return; }
    setEditingId(null);
    router.refresh();
  }

  async function handleDelete(id: number) {
    if (!confirm("削除しますか？")) return;
    const result = await deleteExpenseCategory(id);
    if (result?.error) { setPermError(result.error); return; }
    setItems((prev) => prev.filter((i) => i.id !== id));
  }

  async function handleSort(id: number, direction: "up" | "down") {
    const result = await updateExpenseCategorySortOrder(id, direction);
    if (result?.error) { setPermError(result.error); return; }
    router.refresh();
  }

  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-4">
      <h2 className="text-sm font-semibold text-black mb-4">費目設定（全事業部共通）</h2>
      {permError && <div className="mb-3 px-3 py-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg flex justify-between">{permError}<button onClick={() => setPermError(null)} className="text-red-400">×</button></div>}

      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="費目名"
          className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none"
        />
        <label className="flex items-center gap-1.5 text-sm text-black">
          <input
            type="checkbox"
            checked={isFixed}
            onChange={(e) => setIsFixed(e.target.checked)}
            className="rounded"
          />
          固定費
        </label>
        <button
          onClick={handleAdd}
          disabled={isPending || !name.trim()}
          className="px-3 py-1.5 text-sm text-white rounded-lg disabled:opacity-50"
          style={{ backgroundColor: "#006a38" }}
        >
          追加
        </button>
      </div>

      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-50 text-xs text-black">
            <th className="py-2 px-3 text-left font-medium border border-gray-200">名称</th>
            <th className="py-2 px-3 text-center font-medium border border-gray-200">固定費</th>
            <th className="py-2 px-3 border border-gray-200"></th>
          </tr>
        </thead>
        <tbody>
          {items.length === 0 ? (
            <tr>
              <td colSpan={3} className="py-6 text-center text-black text-sm border border-gray-200">データがありません</td>
            </tr>
          ) : (
            items.map((c, idx) => (
              <tr key={c.id} className="hover:bg-gray-50">
                {editingId === c.id ? (
                  <>
                    <td className="py-1 px-2 border border-gray-200">
                      <input
                        value={editName}
                        onChange={(e) => setEditName(e.target.value)}
                        className="w-full border border-gray-200 rounded px-2 py-1 text-sm bg-white"
                      />
                    </td>
                    <td className="py-1 px-2 text-center border border-gray-200">
                      <input
                        type="checkbox"
                        checked={editIsFixed}
                        onChange={(e) => setEditIsFixed(e.target.checked)}
                        className="rounded"
                      />
                    </td>
                    <td className="py-1 px-2 border border-gray-200">
                      <div className="flex gap-1">
                        <button
                          onClick={() => handleUpdate(c.id)}
                          className="text-xs text-white px-2 py-0.5 rounded"
                          style={{ backgroundColor: "#006a38" }}
                        >
                          保存
                        </button>
                        <button
                          onClick={() => setEditingId(null)}
                          className="text-xs text-black px-2 py-0.5 rounded border border-gray-200"
                        >
                          キャンセル
                        </button>
                      </div>
                    </td>
                  </>
                ) : (
                  <>
                    <td className="py-2 px-3 text-black border border-gray-200">{c.name}</td>
                    <td className="py-2 px-3 text-center text-black border border-gray-200">{c.is_fixed ? "✓" : ""}</td>
                    <td className="py-2 px-3 border border-gray-200">
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => handleSort(c.id, "up")}
                          disabled={idx === 0}
                          className="text-xs text-black hover:text-gray-600 disabled:opacity-30 px-1"
                        >↑</button>
                        <button
                          onClick={() => handleSort(c.id, "down")}
                          disabled={idx === items.length - 1}
                          className="text-xs text-black hover:text-gray-600 disabled:opacity-30 px-1"
                        >↓</button>
                        <button onClick={() => startEdit(c)} className="text-xs text-blue-500 hover:text-blue-700 ml-1">編集</button>
                        <button onClick={() => handleDelete(c.id)} className="text-xs text-red-400 hover:text-red-600 ml-1">削除</button>
                      </div>
                    </td>
                  </>
                )}
              </tr>
            ))
          )}
        </tbody>
      </table>
    </div>
  );
}
