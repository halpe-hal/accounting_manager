...
import { getExpenseCategories, getFixedCategories, getDefaultPartners } from "@/app/actions/settings";
import type { Division, AccountItem, ExpenseCategory, Expense, FixedCategory, DefaultPartner } from "@/lib/types";
...
export async function getRegistrationFormData(): Promise<RegistrationFormData> {
  ...
  const [categories, fixedCategories, defaultPartners, { data: divisions }, { data: accounts }] = await Promise.all([
