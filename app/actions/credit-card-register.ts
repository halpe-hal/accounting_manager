...
import { getExpenseCategories } from "@/app/actions/settings";
import type { Division, AccountItem, ExpenseCategory, Expense } from "@/lib/types";
...
export async function getRegistrationFormData(): Promise<RegistrationFormData> {
  ...
  const [categories, { data: divisions }, { data: accounts }] = await Promise.all([
